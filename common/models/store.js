...
module.exports = function(Store) {
  ...
  Store.getstore = function(req, res, cb) {
    try {
      let db =  Store.dataSource;
      let sql = `SELECT id, user_id, shop_name, store_url, image, tagline, workinghours, description, latitude, longitude, neighbourhood, (SELECT group_concat(CONCAT(COALESCE(cat.id,''), ':', COALESCE(cat.name,''), ':', COALESCE(cat.image_url,'NULL'))SEPARATOR ',') FROM StoreCategory as stc
                 JOIN category as cat
                 on stc.categoty_id = cat.id
                 WHERE stc.store_id = st.id) as category FROM Store as st WHERE st.id = ${req.params.id}`;
      db.connector.execute(sql, function(err, stores) {
        if (err) {
          let error = new Error(err);
          error.status = 400;
